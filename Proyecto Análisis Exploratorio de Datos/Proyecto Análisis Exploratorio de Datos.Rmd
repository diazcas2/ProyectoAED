...
vehiculos_tidy <- vehiculos %>%
  separate(col = TRAM, into = c('Código_CV', 'Num_tramo'), sep = 2)
```
...
Mediante la función pivot_longer(), se crean dos nuevas columnas: $Tipo_Posición_Punto_Quilométrico$, que especifica si la posición corresponde al punto inicial o final, y $Valor_Punto_Quilométrico$, que contiene el valor asociado a dicha posición.

```{r}
vehiculos_tidy <- vehiculos_tidy %>%
   pivot_longer(col = matches('^PK(INI|FI|E)'), names_pattern = '^PK(INI|FI|E)',
                names_to = 'Tipo_Posición_Punto_Quilometrico', 
